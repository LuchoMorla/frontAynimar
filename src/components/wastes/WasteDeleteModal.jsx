...
const WasteDeleteModal = ({ children, waste, setWastes }) => {
  const [showModal, setShowModal] = useState(false);

  const handleClickDelete = async () => {
    try {
      await axios.delete(endPoints.wastes.deleteWastes(waste.id), {
        headers: {
          Authorization: `Bearer ${Cookies.get('token')}`,
        },
      });
      setWastes((wastes) => wastes.filter((w) => w.id !== waste.id));
      toast.success('Materia prima eliminada');
    } catch (err) {
      toast.error('Hubo un error al eliminar la materia prima. Suele pasar cuando tienes ofertas de esta materia prima.');
    }
              ...
              <div className="relative w-auto my-6 mx-auto max-w-3xl">
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                  ...
                  <div className="flex items-start justify-between px-5 py-3 border-b border-solid border-blueGray-200 rounded-t">
                    <h3 className="text-xl font-semibold">Estas a punto de eliminar materia prima</h3>
                    <button
                  ...
                  <div className="relative p-6 flex-auto flex items-center flex-col">
                    <h3 className="font-semibold">Estas seguro de eliminar {`"${waste.name}"`}.</h3>
...
export default WasteDeleteModal;
