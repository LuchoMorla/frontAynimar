...
const WasteEditModal = ({ children, waste, setWastes }) => {
  ...
  const handleSubmitEditModal = async (e) => {
    ...
    const bodyFiltered = Object.fromEntries(Object.entries(body).filter(([key, value]) => value && value !== waste[key]));

    const { data: wasteUploaded } = await axios.patch(endPoints.wastes.updateWastes(waste.id), bodyFiltered);

    setWastes((wastes) => wastes.map((waste) => (waste.id === +wasteUploaded.id ? wasteUploaded.rta : waste)));

    handleClickClose();
  };

  const gettingCategories = async () => {
    const { data } = await axios.get(endPoints.categories.getWastesCategories, {
      headers: {
              ...
              <div className="relative w-auto my-6 mx-auto max-w-3xl">
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                  ...
                  <div className="flex items-start justify-between px-5 py-3 border-b border-solid border-blueGray-200 rounded-t">
                    <h3 className="text-xl font-semibold">Edita la materia prima</h3>
                    <button
                  ...
                  <div className="relative p-6 flex-auto">
                    <form onSubmit={handleSubmitEditModal} id="form-create-waste" className="flex flex-col gap-2">
                      <div>
                        <label htmlFor="name-waste" className="sr-only">
                          Nombre
                        </label>
                        <input
                          defaultValue={waste.name}
                          id="name-waste"
                          name="name"
                          type="text"
                          autoComplete="name"
                      ...
                      <div>
                        <label htmlFor="description-waste" className="sr-only">
                          Descripción
                        </label>
                        <input
                          defaultValue={waste.description}
                          id="description-waste"
                          name="description"
                          type="text"
                          required
                          className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                      ...
                      <div>
                        <label htmlFor="price-waste" className="sr-only">
                          Precio
                        </label>
                        <input
                      ...
                      <div>
                        <label htmlFor="category-waste" className="sr-only">
                          Categoría
                        </label>
                        <select
                          ...
                          required
                          className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                          placeholder="Categoría de la materia prima"
                        >
                          {categories.map((category) => (
                            <option selected={category.id === waste.categoryId} key={category.id} value={category.id}>
                              {category.name}
                            </option>
                          ))}
                      ...
                      <AnimatePresence>
                        {isOther && (
                          ...
                          >
                            <label htmlFor="category-other-waste" className="sr-only">
                              Nueva categoria
                            </label>
                            <input
                              id="category-other-waste"
                              name="categoryOther"
                              type="text"
                              required
                              className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                      ...
                      <Upload textButton="Sube la nueva imagen de la materia prima" modalTitle="Subir la nueva imagen de la materia prima" blobImage={blobImage} />
                    </form>
                  </div>
                  {/*footer*/}
